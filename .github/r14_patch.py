...
TARGET = Path("crates/adapters/src/responses/grok_tool_shim.rs")
...
replace_once(
    ...
    "initial arguments normalization",
)
...
replace_once(
    ...
    '''        if let Some(mut p) = self.items.remove(&output_index) {
            ...
            self.emit_tool_call_done(output_index, &p, false, out);''',
    "output_item.done authoritative arguments",
)
...
replace_once(
    ...
    "fail closed incomplete streaming apply_patch",
)
...
replace_once(
    ...
    "fail closed interrupted terminal envelope",
)
...
replace_once(
    ...
    "envelope arguments normalization",
)
...
replace_once(
    ...
    "fail closed non-stream envelope",
)
...
fn finalize_apply_patch(args_acc: &str, cwd: Option<&str>, interrupted: bool) -> (String, bool) {
    ...
    "argument helpers and double decode",
)
...
TESTS = r'''    #[test]
    ...
    #[test]
    fn tool_search_function_call_rewritten_to_tool_search_call() {'''
replace_once(TEST_ANCHOR, TESTS, "r14 regression tests")

TARGET.write_text(text, encoding="utf-8")

# Keep modified installers visually distinguishable from r13.
for version_path in [
    ...
    if not version_path.exists():
        continue
    value = version_path.read_text(encoding="utf-8")
    value = value.replace("2.4.5+13", "2.4.5+14")
    if version_path.name == "tauri.conf.json":
        value = value.replace("Sub2API Grok Compat r13", "Sub2API Grok Compat r14")
