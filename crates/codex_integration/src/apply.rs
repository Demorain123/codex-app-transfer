...
pub struct ApplyConfig<'a> {
    ...
    /// [correctness review HIGH] 当前 active provider 是否 QoderWork(authScheme `qoder_oauth`)。
    /// QoderWork 的网关 key(`auto`/`gm51model` 等含通用别名,与 WorkBuddy 的 `auto` 撞名)只在
    /// 本 provider 上下文里解析 reasoning 档 + context —— 透传给 catalog 生成,避免污染同名 model。
    /// 其他 provider 为 `false`(行为不变)。见 registry::qoder_catalog::is_qoder_auth_scheme。
    #[serde(default)]
    pub is_qoder: bool,
...
//
// User-authored custom-provider tables remain user-owned and are never rewritten.
// Transfer parses retry/timeout/header fields for diagnostics and future
// Transfer-side compatibility work, but it does not write a fake
// [model_providers.openai] override: stock Codex does not merge configured
// fields over the built-in `openai` provider. r94.1 therefore never claims a
// provider behavior field is effective unless Transfer itself can enforce it.
//
// No Codex binary, app-server, MSIX content, or private runtime is patched or
...
fn log_provider_policy_truth(
    ...
    tracing::info!(
        ...
        reason = reason,
        "provider policy Transfer-only observation"
    );
}
...
pub fn apply_provider(paths: &CodexPaths, cfg: &ApplyConfig) -> Result<ApplyResult, CodexError> {
    ...
    // Provider policy is observed but not rewritten into built-in openai.
    // Stock Codex ignores configured fields that collide with the built-in
    // provider id, so writing model_providers.openai.stream_max_retries=15
    // would be cosmetic and misleading. Keep the source table untouched.
    ...
    // 2b. Provider identity remains built-in openai.
    //
    // The user's source provider table is intentionally left untouched. We do
    // not copy its behavior fields to model_providers.openai because stock
    // Codex ignores configured collisions with the built-in provider.
...
";
    ...
    fn r94_1_provider_policy_truth_reads_custom_provider_fields() {
        ...
        assert!(truth.behavior_fields.iter().any(|f| f == "future_user_field"));
    }

    #[test]
    fn r94_1_provider_policy_reader_accepts_plain_header_comment() {
        let config = "model_provider = \"OpenAi\"\n\n[model_providers.OpenAi] # user provider\nstream_max_retries = 15\n";
        let truth = provider_policy_truth_from_config(config).expect("provider policy");
        assert_eq!(truth.stream_max_retries.as_deref(), Some("15"));
    }

    #[test]
    fn r94_1_provider_policy_parser_marks_quoted_and_nested_forms() {
        let quoted = "model_provider = \"Open Ai\"\n\n[model_providers.\"Open Ai\"]\nstream_max_retries = 15\n";
        let quoted_truth =
            provider_policy_truth_from_config(quoted).expect("quoted provider policy");
        assert!(quoted_truth.section_requires_quoted_key);

        let nested = "model_provider = \"OpenAi\"\n\n[model_providers.OpenAi]\nstream_max_retries = 15\n\n[model_providers.OpenAi.auth]\ncommand = \"helper\"\n";
        let nested_truth =
            provider_policy_truth_from_config(nested).expect("nested provider policy");
        assert!(nested_truth.has_nested_provider_table);
    }
    ...
    fn r94_1_live_provider_switch_in_same_session_fails_before_mutation() {
        ...
        let err = apply_provider(&paths, &cfg)
            .expect_err("same-session provider switch requires a fresh baseline");
        assert!(
            err.to_string()
    ...
    #[test]
    fn r94_1_expected_root_provider_absence_reuses_snapshot_source_without_fake_overlay() {
        let (_t, paths) = setup();
        std::fs::create_dir_all(&paths.codex_home).unwrap();
        std::fs::write(
            &paths.config_toml,
        ...
        assert_eq!(
            snapshot_table_field_literal(&toml, "model_providers.OpenAi", "stream_max_retries")
                .as_deref(),
            Some("15")
        );
        assert!(
            snapshot_table_field_literal(&toml, "model_providers.openai", "stream_max_retries")
                .is_none(),
            "Transfer must not write a cosmetic built-in openai override: {toml}"
        );
    }
    ...
    #[test]
    fn r94_1_provider_policy_is_observed_but_not_copied_to_builtin_openai() {
        let (_t, paths) = setup();
        std::fs::create_dir_all(&paths.codex_home).unwrap();
        ...
        assert_eq!(
            snapshot_table_field_literal(&toml, "model_providers.OpenAi", "request_max_retries")
                .as_deref(),
            Some("7")
        );
        assert!(
            snapshot_table_field_literal(&toml, "model_providers.openai", "stream_max_retries")
                .is_none(),
            "do not claim a stock-Codex override that does not work: {toml}"
        );
        ...
        assert_eq!(
            snapshot_table_field_literal(&restored, "model_providers.OpenAi", "stream_max_retries")
                .as_deref(),
            Some("15")
        );
    }
    ...
    #[test]
    fn r94_1_builtin_openai_table_is_left_user_owned() {
        let (_t, paths) = setup();
        std::fs::create_dir_all(&paths.codex_home).unwrap();
        std::fs::write(
            &paths.config_toml,
