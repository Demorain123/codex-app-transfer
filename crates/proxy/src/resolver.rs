...
/// 判断 Bearer 是否是 OpenAI ChatGPT 的 access_token。
///
...
fn is_chatgpt_access_token(token: &str) -> bool {
    ...
    auth.get("organizations")
        ...
        .is_some_and(|orgs| {
            orgs.iter()
                .any(|org| org.as_object().is_some_and(|o| non_empty_str(o.get("id"))))
        })
}
...
mod tests {
    ...
    fn relay_accepts_any_chatgpt_shaped_jwt() {
        ...
        // ⑤ 当前 OpenAI 也会签发不含 chatgpt_account_id、仅含 user_id + organizations
        ...
        let current_shape = format!("Bearer eyJhbGciOiJub25lIn0.{current_shape_payload}.sig");
