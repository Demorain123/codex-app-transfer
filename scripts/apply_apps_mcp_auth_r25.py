...
if "CAS-APPS-MCP-AUTH-R25-STATE" not in text:
    ...
    # Both constructors must default to no callback. Do this strictly so future
    # constructor drift fails materialization rather than silently leaving one path uninitialised.
...
if "CAS-APPS-MCP-AUTH-R25-REHYDRATE" not in text:
    ...
    # Once r25 injects a bearer, 401 revocation correlation must fingerprint the
    # actual prepared outbound request rather than the originally-authless inbound request.
    text = replace_once(
        text,
...
# Final materialization gate: fail on partial application rather than packaging a
# build whose version says r25 but whose credential path is missing one side.
required = {
    "crates/proxy/src/forward.rs": [
        "CAS-APPS-MCP-AUTH-R25-STATE",
        "CAS-APPS-MCP-AUTH-R25-HELPERS",
