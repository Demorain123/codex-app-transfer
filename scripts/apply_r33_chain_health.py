...
def write(rel: str, body: str) -> None:
    ...
    print(f"r33 wrote {rel}")


def patch_once(rel: str, anchor: str, replacement: str, marker: str = MARKER) -> None:
    path = ROOT / rel
    body = path.read_text(encoding="utf-8")
    if marker in body:
        return
    if anchor not in body:
        raise SystemExit(f"r33 anchor missing in {rel}: {anchor[:120]!r}")
    body = body.replace(anchor, replacement, 1)
    path.write_text(body, encoding="utf-8")
    print(f"r33 patched {rel}")


CHAIN_HEALTH_RS = r'''//! CAS-R33-CHAIN-HEALTH
//!
...
async fn build_snapshot(state: &AdminState) -> ChainHealthSnapshot {
    ...
    let transfer = if proxy_status.running {
        ...
        HealthLayer::new("ok", "transfer_listening", "Transfer 本地转发器正在监听")
            ...
            .fact(format!("requests={} success={} failed={}", stats.total, stats.success, stats.failed))
            .fact(format!(
                "active_provider={}",
    ...
    let runtime = match provider.as_ref() {
        ...
        Some(_) => RuntimeHealth {
            layer: HealthLayer::new("ok", "remote_gateway", "活动网关位于远程主机，不检查本机 Docker")
                .fact("runtime=remote"),
            kind: "remote".into(),
            docker_desktop: None,
...
async fn probe_gateway(target: &ProviderTarget) -> HealthLayer {
    ...
    let resolved = match timeout(DNS_TIMEOUT, lookup_host((target.host.as_str(), target.port))).await {
        Ok(Ok(addrs)) => {
    ...
    let client = match reqwest::Client::builder()
        ...
        .user_agent(concat!("Codex-App-Transfer-Chain-Health/", env!("CARGO_PKG_VERSION")))
        .build()
    {
        Ok(client) => client,
        Err(error) => {
            return HealthLayer::new("degraded", "gateway_http_client_failed", "TCP 可达，但 HTTP 探针初始化失败")
                .fact(format!("error={}", compact_error(&error.to_string())));
        }
    };
    ...
    match response {
        Ok(response) => {
            ...
            let (level, code, summary) = match status {
                401 | 403 => ("degraded", "gateway_auth_required", "网关可达，但鉴权未通过或未携带凭据"),
                429 => ("degraded", "gateway_rate_limited", "网关可达，但当前处于限流状态"),
                500..=599 => ("error", "gateway_http_5xx", "网关返回 5xx，应用或其依赖可能异常"),
                _ => ("ok", "gateway_http_reachable", "网关 DNS、TCP 与 HTTP 均可响应"),
            };
            HealthLayer::new(level, code, summary)
                .latency(Some(http_ms))
                .fact(format!("http_status={status}"))
                .fact(format!("dns_ms={dns_ms} tcp_ms={tcp_ms} http_headers_ms={http_ms}"))
...
async fn probe_local_runtime(target: &ProviderTarget, gateway: &HealthLayer) -> RuntimeHealth {
    let desktop = run_command(
        "docker",
        &["desktop".into(), "status".into(), "--format".into(), "json".into()],
    ...
    let info = run_command(
        "docker",
        &["info".into(), "--format".into(), "{{json .ServerVersion}}".into()],
        COMMAND_TIMEOUT,
    )
    .await;
    match info.kind {
        ...
        CommandKind::NotFound => return native_runtime(target, gateway, "docker_cli_not_found").await,
    ...
    if !matches!(target_ids_result.kind, CommandKind::Ok) {
        return RuntimeHealth {
            layer: HealthLayer::new("degraded", "docker_container_query_failed", "Docker Engine 可达，但容器查询失败")
    ...
    if let Some(project) = compose_project.as_deref() {
        ...
        if matches!(project_ids.kind, CommandKind::Ok) {
            for id in project_ids.stdout.lines().map(str::trim).filter(|v| !v.is_empty()) {
    ...
    for value in inspected {
        ...
        let stat = stats
            .get(&id)
            .or_else(|| stats.iter().find(|(key, _)| id.starts_with(key.as_str())).map(|(_, v)| v));
        containers.push(DockerContainerHealth {
            target: target_prefixes.iter().any(|prefix| id.starts_with(*prefix) || prefix.starts_with(&id)),
            id: id.chars().take(12).collect(),
            name,
            service: compose_service_of(&value),
            running: state.get("Running").and_then(Value::as_bool).unwrap_or(false),
            status: state
                .get("Status")
                .and_then(Value::as_str)
                .unwrap_or("unknown")
                .to_owned(),
            health,
            restarting: state.get("Restarting").and_then(Value::as_bool).unwrap_or(false),
            oom_killed: state.get("OOMKilled").and_then(Value::as_bool).unwrap_or(false),
            exit_code: state.get("ExitCode").and_then(Value::as_i64).unwrap_or(0),
            restart_count: value.get("RestartCount").and_then(Value::as_u64).unwrap_or(0),
            cpu: stat.and_then(|v| v.get("CPUPerc")).and_then(Value::as_str).map(ToOwned::to_owned),
            memory: stat.and_then(|v| v.get("MemUsage")).and_then(Value::as_str).map(ToOwned::to_owned),
            pids: stat.and_then(|v| v.get("PIDs")).and_then(Value::as_str).map(ToOwned::to_owned),
        });
    }
...
async fn native_runtime(target: &ProviderTarget, gateway: &HealthLayer, reason: &str) -> RuntimeHealth {
    ...
    let (status, code, summary) = if reachable {
        ("ok", "native_runtime_reachable", "本机端口由非 Docker 服务提供，网关探针可达")
    } else if owner.is_some() {
        ("degraded", "native_runtime_unresponsive", "本机进程正在监听端口，但网关 HTTP 不正常")
    } else {
        ("unknown", "native_runtime_unknown", "未发现映射该端口的 Docker 容器或可识别本机进程")
    };
...
async fn native_port_owner(port: u16) -> Option<(u32, String)> {
    ...
    let pid = result.stdout.lines().find_map(|line| {
        ...
        (local_port == port).then(|| cols[4].parse::<u32>().ok()).flatten()
    })?;
...
fn codex_layer() -> HealthLayer {
    #[cfg(target_os = "windows")]
    {
        ...
        let codex = rows
            ...
            .count();
        if chatgpt + codex == 0 {
            return HealthLayer::new("idle", "codex_not_running", "未检测到 Codex Desktop / app-server 进程");
        }
    ...
    #[cfg(not(target_os = "windows"))]
    HealthLayer::new("unknown", "codex_probe_windows_only", "当前版本仅在 Windows 提供 Codex 原生进程探针")
}
...
fn passive_upstream_layer() -> HealthLayer {
    ...
    let Some((forward_index, forward_time)) = last_forward else {
        return HealthLayer::new("idle", "upstream_no_requests", "尚无可用于判断上游的真实请求证据")
    ...
    if forward_index > status_index {
        let level = if age >= 90 { "error" } else if age >= 20 { "degraded" } else { "ok" };
        let code = if age >= 90 {
    ...
    if status_index > timing_index {
        return HealthLayer::new("ok", "upstream_streaming", "已收到响应头，流式响应仍在进行或尚未记录收尾")
            .latency(Some(age.saturating_mul(1000)))
            .fact("evidence=proxy-log-order-best-effort");
    }
    if let Some((error_index, error)) = last_error {
        if error_index > timing_index {
            return HealthLayer::new("error", "upstream_recent_error", "最近一次请求在 Transfer/上游阶段失败")
                .fact(format!("error={error}"));
        }
    }
    if let Some((_, _, status)) = last_status {
        let (level, code, summary) = match status {
            401 | 403 => ("degraded", "upstream_auth_error", "最近请求到达上游，但鉴权失败"),
            429 => ("degraded", "upstream_rate_limited", "最近请求被限流或账号配额不足"),
            500..=599 => ("error", "upstream_5xx", "最近请求收到网关/上游 5xx"),
            _ => ("ok", "upstream_recent_complete", "最近请求已获得响应并记录收尾时间"),
        };
        return HealthLayer::new(level, code, summary)
            .fact(format!("http_status={status}"))
            .fact("mode=passive-no-inference");
    }
    HealthLayer::new("unknown", "upstream_evidence_incomplete", "检测到转发记录，但上游证据不完整")
}
...
fn recommendations(
    ...
    match gateway.code.as_str() {
        "gateway_tcp_timeout" | "gateway_tcp_refused" => out.push(
            "检查活动 provider 的端口、容器端口映射和本机防火墙。".into(),
        ),
        ...
        "gateway_rate_limited" => out.push(
            "检查网关账号池的额度、RPM/TPM、冷却和已用尽账号是否仍被调度。".into(),
        ),
        "gateway_http_5xx" => out.push(
            "网关返回 5xx：查看网关及其数据库/Redis日志，再判断是否为真正上游故障。".into(),
        ),
        _ => {}
    }
    ...
    if out.is_empty() {
        out.push("自动探针未发现明确故障；用全新 Codex 会话发送一次极小请求，再观察被动上游状态。".into());
    }
    out
}
...
mod tests {
    ...
    fn upstream_status_parser_is_strict() {
        assert_eq!(parse_upstream_status("upstream status 200 http://x"), Some(200));
        assert_eq!(parse_upstream_status("something status 200"), None);
    }

    #[test]
    fn compose_oneoff_is_ignored() {
        assert!(is_compose_oneoff(&json!({
...
CHAIN_HEALTH_TS = r'''// CAS-R33-CHAIN-HEALTH
...
write("src-tauri/src/admin/handlers/chain_health.rs", CHAIN_HEALTH_RS)
write("frontend/src/api/chainHealth.ts", CHAIN_HEALTH_TS)
...
cargo_body = cargo.read_text(encoding="utf-8")
if '"process"' not in cargo_body.split('tokio =', 1)[1].split('\n', 1)[0]:
    anchor = 'tokio = { version = "1", features = ["sync", "net", "rt-multi-thread", "time"] }'
    replacement = 'tokio = { version = "1", features = ["sync", "net", "process", "rt-multi-thread", "time"] } # CAS-R33-CHAIN-HEALTH'
    if anchor not in cargo_body:
        raise SystemExit("r33 tokio feature anchor missing")
    cargo.write_text(cargo_body.replace(anchor, replacement, 1), encoding="utf-8")
    print("r33 enabled tokio process feature")
...
'''
    page = page.replace("</style>", chain_style + "\n</style>", 1)
    proxy_page.write_text(page, encoding="utf-8")
    print("r33 patched frontend/src/pages/ProxyPage.vue")
...
for locale, values in {
    ...
    "en": {
        "title": "Chain Health",
        "loading": "Collecting quota-free diagnostic evidence…",
        "refresh": "Check now",
        "details": "Details",
        "recommendations": "Recommended actions",
        "containers": "Docker / Compose containers",
        "privacy": "Privacy boundary",
        "loadFailed": "Chain health check failed",
        "codex": "Codex",
        "transfer": "Transfer",
        "gateway": "Gateway",
        "runtime": "Runtime",
        "upstream": "Upstream (passive)",
        "ok": "Healthy",
        "degraded": "Degraded",
        "error": "Failed",
        "unknown": "Unknown",
    ...
    path = ROOT / f"frontend/src/i18n/{locale}.ts"
    body = path.read_text(encoding="utf-8")
    if '"chainHealth.title"' in body:
        continue
    anchor = '  "providerForm.grokCompatSection"'
    index = body.find(anchor)
    if index < 0:
        raise SystemExit(f"r33 i18n anchor missing: {locale}")
    entries = [
        f'  "chainHealth.title": {values["title"]!r},',
        f'  "chainHealth.loading": {values["loading"]!r},',
        f'  "chainHealth.refresh": {values["refresh"]!r},',
        f'  "chainHealth.details": {values["details"]!r},',
        f'  "chainHealth.recommendations": {values["recommendations"]!r},',
        f'  "chainHealth.containers": {values["containers"]!r},',
        f'  "chainHealth.privacy": {values["privacy"]!r},',
        f'  "chainHealth.loadFailed": {values["loadFailed"]!r},',
        f'  "chainHealth.layer.codex": {values["codex"]!r},',
        f'  "chainHealth.layer.transfer": {values["transfer"]!r},',
        f'  "chainHealth.layer.gateway": {values["gateway"]!r},',
        f'  "chainHealth.layer.runtime": {values["runtime"]!r},',
        f'  "chainHealth.layer.upstream": {values["upstream"]!r},',
        f'  "chainHealth.status.ok": {values["ok"]!r},',
        f'  "chainHealth.status.degraded": {values["degraded"]!r},',
        f'  "chainHealth.status.error": {values["error"]!r},',
        f'  "chainHealth.status.unknown": {values["unknown"]!r},',
        f'  "chainHealth.status.idle": {values["idle"]!r},',
    ]
    # Python repr uses single quotes; TypeScript accepts them in object values.
    body = body[:index] + "\n".join(entries) + "\n" + body[index:]
    path.write_text(body, encoding="utf-8")
    print(f"r33 patched frontend/src/i18n/{locale}.ts")

print("r33 chain health overlay: COMPLETE")
