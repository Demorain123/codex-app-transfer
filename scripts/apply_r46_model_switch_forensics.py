...
MARKER = "CAS-R46-MODEL-SWITCH-FORENSICS"

source = FORWARD.read_text(encoding="utf-8")
if MARKER in source:
    print("r46 model-switch forensics already applied")
    raise SystemExit(0)

helper_anchor = "pub async fn forward_handler(\n"
if helper_anchor not in source:
    raise SystemExit("r46 forensics: forward_handler anchor missing")

helpers = r'''
// CAS-R46-MODEL-SWITCH-FORENSICS
// Privacy-bounded structural diagnostics for long cross-model threads. This intentionally
// never records prompt/response text, tool arguments, raw thread/session ids, credentials,
// encrypted reasoning contents, or attachment contents. It records only shape/counts,
// model labels, byte lengths, non-reversible fingerprints, and status transitions.
...
fn request_kind_r46(headers: &HeaderMap, body: &[u8]) -> String {
    if let Some(kind) = turn_metadata_r46(headers)
        .as_ref()
    ...
    if is_compaction_helper_request_r45(body) {
        return "compaction".to_owned();
    }
    "turn".to_owned()
}

fn is_compaction_request_r46(headers: &HeaderMap, body: &[u8]) -> bool {
    // x-codex-turn-metadata is authoritative when present. A beta feature flag such as
    // remote_compaction_v2 only advertises capability and must NOT classify a normal turn.
    if let Some(kind) = turn_metadata_r46(headers)
        .as_ref()
        .and_then(|value| value.get("request_kind"))
        .and_then(serde_json::Value::as_str)
    ...
    is_compaction_helper_request_r45(body)
}
...
fn input_type_counts_r46(value: &serde_json::Value) -> (usize, usize, usize, usize, usize, usize, String) {
...
fn analyze_request_forensics_r46(
    ...
    let (input_items, message_items, reasoning_items, compaction_items, toolish_items, unknown_items, input_types) =
        parsed
            .as_ref()
...
fn log_request_forensics_r46(ctx: &RequestForensicsR46) {
    // Keep normal small same-model turns quiet. Long history, model transition and compaction
    // are the only events useful to diagnose this failure family.
...
old_ctx = '''    let r45_compaction_helper = is_compaction_helper_request_r45(&body_bytes);
...
new_ctx = '''    let r45_conversation_fingerprint = conversation_fingerprint_r45(&parts.headers);
    ...
    // r46: request_kind from x-codex-turn-metadata is authoritative. The generic
    // remote_compaction_v2 feature flag is only capability advertisement and does not
    // make an ordinary turn a compaction request.
    let r45_compaction_helper = is_compaction_request_r46(&parts.headers, &body_bytes);
'''
if old_ctx not in source:
    raise SystemExit("r46 forensics: r45 compaction context anchor missing")
...
if resolved_anchor not in source:
    raise SystemExit("r46 forensics: resolved model anchor missing")
...
if result_anchor not in source:
    raise SystemExit("r46 forensics: client status anchor missing")
...
'''
tests = r'''    #[test]
    fn r46_request_kind_header_outranks_beta_feature_noise() {
        let mut headers = HeaderMap::new();
        headers.insert(
            "x-codex-turn-metadata",
            r#"{"request_kind":"turn"}"#.parse().unwrap(),
        );
        assert!(!is_compaction_request_r46(
            &headers,
            br#"{"metadata":{"feature":"remote_compaction_v2"},"input":[]}"#,
        ));
        headers.insert(
            "x-codex-turn-metadata",
        ...
        assert!(is_compaction_request_r46(&headers, br#"{"input":[]}"#));
    }
...
if test_anchor not in source:
    raise SystemExit("r46 forensics: test anchor missing")
...
for invariant in (
    ...
    if invariant not in source:
        raise SystemExit(f"r46 forensics invariant missing: {invariant}")

FORWARD.write_text(source, encoding="utf-8")
print("R46 MODEL-SWITCH FORENSICS PASS")
