...
def run(rel: str) -> None:
    path = ROOT / rel
    if not path.is_file():
        raise SystemExit(f"r53 fast-current-tree required component missing: {rel}")
    print(f"r53 fast-current-tree applying {rel}")
    try:
...
def has_complete_r52_generated_baseline() -> bool:
    ...
    return (
        ...
        and "CAS-R52-SUB2API-CROSS-MODEL-COMPACTION" in responses
...
if has_complete_r52_generated_baseline():
    print("R53 FAST BASELINE: complete generated r52 tree detected; R52 COMPOSITION SKIP")
else:
    print("R53 FAST BASELINE: r52 generated markers incomplete; repairing r52 baseline once")
    run("scripts/apply_r52_fast_current_tree.py")
    if not has_complete_r52_generated_baseline():
        raise SystemExit("r53 fast baseline repair completed but required r52 markers are still missing")
...
if "compat_revision=53" not in version_before or "app_version=2.4.5+53" not in version_before:
    REVISION.write_text("53\n", encoding="utf-8")
    run("scripts/apply_sub2api_grok_compat_revision.py")
else:
    print("R53 version already stamped; revision materializer SKIP")
...
for rel, markers in checks.items():
    ...
    for marker in markers:
        if marker not in text:
            raise SystemExit(f"r53 fast-current-tree invariant missing in {rel}: {marker}")

version = VERSION.read_text(encoding="utf-8")
if "compat_revision=53" not in version or "app_version=2.4.5+53" not in version:
    raise SystemExit("r53 fast-current-tree version stamp missing")

print("R53 FAST CURRENT-TREE COMPOSITION PASS")
print("- complete generated r52 tree is reused without replay when warm")
print("- Sub2API non-Grok compact removes max_output_tokens before forwarding")
print("- Grok compact, ordinary model turns, session identity, and r49 launch TEMP behavior are preserved")
