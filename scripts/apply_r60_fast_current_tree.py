...
def has_complete_r59_generated_baseline() -> bool:
    paths = (BACKEND, ADMIN, PAGE, API, PROCESS, COMPACT, MCP_SERVERS, NO_MICRO, CARGO)
    ...
    return (
        "CAS-R59-INTERRUPTED-TAIL-SAME-ID-RECOVERY" in backend
        and '"rewindInterruptedTail"' in backend
        and "stage=bad_tail_removed" in backend
        and "CAS-R59-INTERRUPTED-TAIL-SAME-ID-RECOVERY" in page
        and "rewindInterruptedTail" in api
        and "/api/thread-recovery/action" in admin
...
print("- only recovery session catalog/lifecycle state is added")
print("- r59 recovery mutation semantics and all earlier fixes remain unchanged")
