...
# Require a completely clean visible working tree (tracked + non-ignored untracked).
# This lets the gate safely remove only files generated by the historical r39 probe.
$dirtyBefore = @(& git status --porcelain --untracked-files=normal)
if ($LASTEXITCODE -ne 0) {
    throw 'Unable to inspect Git working-tree state before r40 validation.'
}
if ($dirtyBefore.Count -gt 0) {
    Write-Host 'r40 validation found pre-existing working-tree entries:' -ForegroundColor Yellow
    $dirtyBefore | ForEach-Object { Write-Host "  $_" -ForegroundColor Yellow }
    throw "r40 validation requires a clean working tree before the environment probe. Preserve any edits you need, then clean/reset the listed entries."
}
...
# necessarily invokes the r39 fast gate and creates both tracked mutations and
# generated non-ignored source files. Because the baseline was verified completely
# clean, everything visible after the probe is probe-generated and can be discarded.
...
# Remove only non-ignored untracked files. The pre-probe tree was verified clean, so
# these are generated by the probe/composition chain rather than pre-existing work.
...
$dirtyAfter = @(& git status --porcelain --untracked-files=normal)
if ($LASTEXITCODE -ne 0) {
    throw 'Unable to verify Git working-tree state after r40 baseline restore.'
}
if ($dirtyAfter.Count -gt 0) {
    throw 'r40 baseline restore left visible changes behind; refusing to compose on a partially materialized tree.'
}
Write-Host 'r40 source baseline: CLEAN (probe environment retained; tracked + generated source mutations discarded)' -ForegroundColor Green
