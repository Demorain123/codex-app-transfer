...
function Find-LibClang {
    if ($env:LIBCLANG_PATH -and (Test-Path -LiteralPath $env:LIBCLANG_PATH -PathType Container)) {
        $existing = Get-ChildItem -LiteralPath $env:LIBCLANG_PATH -Filter 'libclang.dll' -File -ErrorAction SilentlyContinue | Select-Object -First 1
        if (-not $existing) { $existing = Get-ChildItem -LiteralPath $env:LIBCLANG_PATH -Filter 'clang.dll' -File -ErrorAction SilentlyContinue | Select-Object -First 1 }
    ...
    $roots = @(
        ...
        (Join-Path ${env:ProgramFiles(x86)} 'Microsoft Visual Studio\2022\BuildTools\VC\Tools\Llvm'),
        (Join-Path ${env:ProgramFiles(x86)} 'Microsoft Visual Studio\2022\BuildTools\VC\Tools\Llvm\x64')
    )
    foreach ($root in $roots) {
        if (-not $root -or -not (Test-Path -LiteralPath $root -PathType Container)) { continue }
        $found = Get-ChildItem -LiteralPath $root -Filter 'libclang.dll' -File -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
        if (-not $found) { $found = Get-ChildItem -LiteralPath $root -Filter 'clang.dll' -File -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1 }
        if ($found) { return $found.FullName }
    }
    $null
}
...
if (-not $rustupCmd) { throw 'rustup was not found in PATH.' }
...
Write-Host "`n[5/9] Ensure CMake for boring-sys2" -ForegroundColor Green
$cmake = Find-CMake
if (-not $cmake) {
...
} else { Write-Host "CMake already available; SKIP install: $cmake" -ForegroundColor Green }
...
$nasm = Find-Nasm
if (-not $nasm) { throw 'NASM is still missing. Run scripts\Run-r46-FAST-REAL-USE.cmd so the portable NASM preflight can provision it.' }
...
Write-Host "`n[7/9] Ensure LLVM/libclang for bindgen" -ForegroundColor Green
$libclang = Find-LibClang
if (-not $libclang) {
    if (-not $winget) { throw 'libclang is required by bindgen, but LLVM was not found and winget is unavailable.' }
    Write-Host 'libclang missing; installing LLVM.LLVM once with winget...' -ForegroundColor Yellow
    Invoke-Checked $winget 'install' '--id' 'LLVM.LLVM' '--exact' '--silent' '--accept-package-agreements' '--accept-source-agreements'
    $machinePath = [Environment]::GetEnvironmentVariable('Path', 'Machine')
    $userPath = [Environment]::GetEnvironmentVariable('Path', 'User')
    if ($machinePath -or $userPath) { $env:PATH = "$machinePath;$userPath;$env:PATH" }
    $libclang = Find-LibClang
    if (-not $libclang) { throw 'LLVM install completed, but libclang.dll/clang.dll was still not found.' }
} else {
    Write-Host "libclang already available; SKIP install: $libclang" -ForegroundColor Green
}
...
Write-Host "Using LIBCLANG_PATH: $libclangDir" -ForegroundColor Green
