...
if (-not (Test-Path $LibClang)) {
    throw "r72 build requires shared libclang at: $LibClang"
}
...
Write-Host "[r72] VS=$VS_HOME"
Write-Host "[r72] LIBCLANG_PATH=$env:LIBCLANG_PATH"
Write-Host "[r72] expected visible identity: Sub2API Grok Compat r72 / v2.4.5+72"
...
if (-not $SkipFocusedTests) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "r72 historical-turn recovery focused test failed" }

    cargo test -p codex-app-transfer-proxy telemetry
    if ($LASTEXITCODE -ne 0) { throw "r72 observability focused test failed" }
}

$Exe = Join-Path $RepoRoot "target\release\codex-app-transfer.exe"
$DeployExe = Join-Path $DeployDir "codex-app-transfer.exe"

# Windows locks a running executable. Stop only this repo/deploy copy so the
# replacement is deterministic; do not kill unrelated Transfer installations.
...
if ($Running.Count -gt 0) {
    Write-Host "[r72] stopping running Transfer before rebuild"
...
Write-Host ""
Write-Host "R72_LOCAL_BUILD_PASS"
...
if (-not $NoDeploy) {
    ...
    if ($RunningDeploy.Count -gt 0) {
        Write-Host "[r72] stopping running deployed Transfer before replacement"
