...
r60_guard_tail = r59_guard_tail + " -and ((Get-Content -LiteralPath (Join-Path $repoRoot 'crates\\proxy\\src\\forward.rs') -Raw -Encoding UTF8) -match 'CAS-R60-SUB2API-POST-COMPACTION-REPLAY') -and ((Get-Content -LiteralPath (Join-Path $repoRoot 'crates\\proxy\\src\\forward.rs') -Raw -Encoding UTF8) -match 'CAS-R60-POST-COMPACTION-BORROW-SAFETY')"
