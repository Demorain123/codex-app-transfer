...
r60_guard_tail = " -and ((Get-Content -LiteralPath (Join-Path $repoRoot 'src-tauri\\src\\admin\\handlers\\thread_recovery.rs') -Raw -Encoding UTF8) -match 'CAS-R59-INTERRUPTED-TAIL-SAME-ID-RECOVERY') -and ((Get-Content -LiteralPath (Join-Path $repoRoot 'crates\\adapters\\src\\mapper\\sub2api_grok_compat.rs') -Raw -Encoding UTF8) -match 'CAS-R60-SUB2API-POST-COMPACTION-REPLAY') -and ((Get-Content -LiteralPath (Join-Path $repoRoot 'crates\\adapters\\src\\mapper\\responses.rs') -Raw -Encoding UTF8) -match 'CAS-R60-SUB2API-POST-COMPACTION-REPLAY-HOOK')"
r61_guard_tail = r60_guard_tail + " -and ((Get-Content -LiteralPath (Join-Path $repoRoot 'crates\\proxy\\src\\forward.rs') -Raw -Encoding UTF8) -match 'CAS-R61-MODEL-SWITCH-COMPACT-RESUME-ONCE')"
if r60_guard_tail not in text:
    raise SystemExit("r61 fast builder: r60 materialized guard tail missing")
text = text.replace(r60_guard_tail, r61_guard_tail, 1)
...
print("- reused the proven r60/r59/r58/r57/r56/r55/r54/r53/r52/r51/r50/r49/r48/r47/r46 DevCache and toolchain")
