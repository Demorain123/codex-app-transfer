...
# Executed inside the generated r88/r86 builder after r86 has produced the
# temporary r75 source. It injects the pane UI change into the r75 builder so
# the actual r74 telemetry runtime owner is changed in-memory after clean gates.
...
# Outer payload uses a literal single-quoted here-string. Any here-string that
# must exist inside the generated r75 script deliberately uses the opposite
# (double-quoted) delimiter so PowerShell cannot terminate this payload early.
$R88R75RuntimePatch = @'
# R88_PANE_RUNTIME_PATCH
$R88StatusConstantOld = "  const STATUS_ID = 'cas-live-statusbar';"
$R88StatusConstantNew = @"
  const STATUS_ID = 'cas-live-statusbar';
  const PANE_STATUS_ATTR = 'data-cas-pane-statusbar';
  const PANE_THREAD_ATTR = 'data-cas-pane-thread-id';
"@
...
$R88InsideOwnNew = @"
    return !!node.closest('#' + STATUS_ID + ',#' + MIRROR_ID + ',#' + ANALYTICS_ID + ',[' + PANE_STATUS_ATTR + '=true]');
"@
...
$R88ComposerBlock = @"
  ...
  function normalizePaneThreadId(value) {
    return String(value || '').replace(/^local:/i, '').replace(/^agent-/i, '').trim().toLowerCase();
  }
  ...
  function paneThreadId(pane, composer) {
    ...
    if (scope instanceof Element) {
      ...
      const headerText = normalizedText(scope).slice(0, 500);
      const agent = headerText.match(/\bagent-([0-9a-f]{8,36})\b/i);
      if (agent) return normalizePaneThreadId(agent[1]);
    }
    const composers = findComposerRoots();
  ...
  function statusHtmlForPane(threadId) {
    ...
    const escapedTitle = sidTitle.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\"/g, '&quot;');
    const sid = '<span class=\"cas-status-item cas-status-muted\" title=\"' + escapedTitle + '\">sid ' + shortPaneThreadId(paneId) + '</span>';
    if (ownsExactMetrics || singleUnknown) return statusHtml() + sid;
    return [
...
$R88RefreshBlock = @"
  function refreshUi() {
    ...
    for (const bar of bars) {
      ...
      bar.innerHTML = statusHtmlForPane(threadId);
      let width = 9999;
...
$R88CleanupNew = @"
    document.querySelectorAll('[' + PANE_STATUS_ATTR + '=true]').forEach(function(node) { node.remove(); });
