...
# temporary r75 source. It injects a no-extra-network, pane-scoped UI patch into
# the r75 builder so the actual r74 telemetry runtime owner is modified in-memory
# without touching the tracked r74 source before clean-worktree gates.
...
$R88ComposerBlock = @'
  ...
  function statusHtmlForPane(threadId) {
    ...
    const sid = '<span class="cas-status-item cas-status-muted" title="' + sidTitle.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\"/g, '&quot;') + '">sid ' + shortPaneThreadId(paneId) + '</span>';
