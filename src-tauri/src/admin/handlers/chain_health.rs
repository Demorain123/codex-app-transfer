...
async fn build_snapshot(state: &AdminState) -> ChainHealthSnapshot {
    ...
    let recommendations = recommendations(
        &session, &mcp, &transfer, &gateway, &runtime, &account, &upstream, &diagnosis,
    );
...
fn account_pool_layer_r37(upstream: &HealthLayer) -> HealthLayer {
    ...
    let quota = records.iter().rev().find(|record| {
        record.quota_primary_used_percent.is_some() || record.quota_secondary_used_percent.is_some()
    });
...
fn fault_attribution_layer_r37(
    ...
    if let Some(record) = failed {
        let compact_signal =
            recent_log_age_r37("\"request_kind\":\"compaction\"", R37_EVIDENCE_WINDOW_SECS)
